...
function addComment({ photoId, author, text }) {
  const db = load();
  const photoExists = db.photos.some((p) => p.id === photoId);
  if (!photoExists) throw new Error('That photo does not exist');
  const comment = {
    id: crypto.randomUUID(),
    photoId,
    author,
...
function deletePhoto(id, person) {
  ...
  db.comments = db.comments.filter((c) => c.photoId !== id);
  save(db);
  try {
...
function castVote({ date, voter, photoId }) {
  ...
  if (!photoExists) throw new Error('That photo does not exist for this day');
